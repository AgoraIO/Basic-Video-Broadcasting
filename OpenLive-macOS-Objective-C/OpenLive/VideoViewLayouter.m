...
- (VideoSession *)responseSessionOfEvent:(NSEvent *)gesture
    ...
    CGPoint location = gesture.locationInWindow;
    for (VideoSession *session in sessions) {
...
- (NSArray<NSView *> *)viewListFromSessions:(NSArray<VideoSession *> *)sessions maxCount:(NSUInteger)maxCount ignorSession:(VideoSession *)ignorSession {
...
- (NSArray<NSLayoutConstraint *> *)layoutFullScreenView:(NSView *)view inContainerView:(NSView *)container {
...
- (NSArray<NSLayoutConstraint *> *)layoutSmallViews:(NSArray<NSView *> *)smallViews inContainerView:(NSView *)container {
    NSMutableArray *layouts = [[NSMutableArray alloc] init];
    NSView *lastView;
    
    CGFloat itemSpace = 5;
    
    for (NSView *view in smallViews) {
        [container addSubview:view];
...
- (NSArray<NSLayoutConstraint *> *)layoutGridViews:(NSArray<NSView *> *)allViews inContainerView:(NSView *)container {
    NSMutableArray *layouts = [[NSMutableArray alloc] init];
    ...
    if (viewCount == 1) {
        ...
        NSView *firstView = allViews.firstObject;
        NSView *lastView = allViews.lastObject;
        ...
        NSArray<NSLayoutConstraint *> *v = [NSLayoutConstraint constraintsWithVisualFormat:@"V:|[view1]-3-[view2]|" options:0 metrics:nil views:@{@"view1": firstView, @"view2": lastView}];
