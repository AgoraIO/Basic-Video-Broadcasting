...
private extension RoomViewController {
    ...
    func startShareWindow(_ window: Window) {
        ...
        if windowId == 0 {
            agoraKit.startScreenCaptureByDisplayId(UInt(CGMainDisplayID()), rectangle: CGRect.zero, parameters: AgoraScreenCaptureParameters())
        } else {
            agoraKit.startScreenCaptureByWindowId(UInt(windowId), rectangle: CGRect.zero, parameters: AgoraScreenCaptureParameters())
        }
